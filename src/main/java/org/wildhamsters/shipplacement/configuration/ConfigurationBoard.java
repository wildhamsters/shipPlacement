 ...
 * Contains additional two columns and rows that constitute "frame" around a
 * regular game board.
...
class ConfigurationBoard {
    ...
    /**
     * Places ship on a board. No other subsequently added ship can interfere with
    ...
    private boolean isEnoughSpaceForShipWithinBoardBounds(int requiredSpace, int gameBoardPosition,
    ...
    private Map<Integer, FieldState> fillBoard() {
        ...
        IntStream.range(1, structure.height())
                .forEach(i -> {
                    newBoard.put(structure.rowBegin(i), FieldState.WALL);
